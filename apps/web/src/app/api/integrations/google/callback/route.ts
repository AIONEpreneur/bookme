...
export async function GET(request: Request) {
  try {
    ...
    if (!session || !code || code.length > 2048 || !state || !/^[A-Za-z0-9_-]{32,128}$/.test(state)) throw new AppError("INVALID_OAUTH_CALLBACK", "Die Google-Autorisierung konnte nicht bestätigt werden.", 400);
