...
export async function GET(request: Request, context: Context) {
  try {
    ...
    if (!Number.isFinite(from.getTime()) || !Number.isFinite(to.getTime()) || to <= from || to.getTime() - from.getTime() > 31 * 86400000) throw new AppError("INVALID_RANGE", "Wähle einen gültigen Zeitraum von höchstens 31 Tagen.");
    try { Intl.DateTimeFormat(undefined, { timeZone }); } catch { throw new AppError("INVALID_TIME_ZONE", "Wähle eine gültige Zeitzone."); }
