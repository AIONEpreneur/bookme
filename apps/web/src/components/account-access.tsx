...
function GenericRequestForm({ kind }: { kind: "password" | "verification" }) {
  ...
  if (accepted) return <div className="recovery-result" role="status" aria-live="polite"><strong>Anfrage angenommen</strong><p>Wenn die Adresse berechtigt ist, stellt BookMe Anweisungen über den konfigurierten E-Mail-Anbieter bereit. Diese Seite bestätigt weder ein Konto noch eine Zustellung.</p><Link className="button button-primary" href="/dashboard">Zurück zur Anmeldung</Link></div>;
...
export function AcceptInvitationView() {
  ...
  if (status === "accepted") return <AccessFrame eyebrow="Einladung angenommen" title="Du bist dabei" description="Dieser Workspace ist jetzt über dein Konto verfügbar."><Link className="button button-primary" href="/dashboard">BookMe öffnen</Link></AccessFrame>;
