...
function BookingCard({ booking, branding }: { booking: BookingSummary; branding: WorkspaceBranding | null }) {
  return <section className="confirmation-card"><div className="confirmation-brand"><BrandLogo branding={branding} /><div><strong>{booking.eventTitleSnapshot}</strong><span>{branding?.workspaceName || "BookMe-Buchung"}</span></div></div><dl><div><dt><Icon name="calendar" />Datum und Uhrzeit</dt><dd>{bookingDate(booking)} um {bookingTime(booking)} Uhr<small>{booking.durationMinutes} Minuten · {booking.inviteeTimeZone}</small></dd></div><div><dt><Icon name="video" />Ort</dt><dd>{bookingLocation(booking)}<small>Dein Meeting-Ort</small></dd></div><div><dt><Icon name="video" />Kalender</dt><dd>{booking.calendarSyncStatus.toLowerCase()}<small>{calendarDelivery(booking)}</small></dd></div>{booking.priceCents > 0 && <div><dt><Icon name="sparkles" />Zahlung</dt><dd>${(booking.priceCents / 100).toFixed(2)} {booking.currency.toUpperCase()}<small>{booking.status === "CONFIRMED" ? "Bestätigt" : booking.status === "PENDING_PAYMENT" ? "Zahlung ausstehend" : booking.status.replaceAll("_", " ").toLowerCase()}</small></dd></div>}</dl></section>;
}
...
function BrandHeader({ branding }: { branding: WorkspaceBranding | null }) { return <header className="public-header"><div className="booking-brand"><BrandLogo branding={branding} /><div><strong>{branding?.workspaceName || "BookMe"}</strong><span>{branding?.description || "Sichere Terminplanung"}</span></div></div></header>; }
function PublicFooter({ branding }: { branding: WorkspaceBranding | null }) { return <footer className="public-footer"><span>{branding?.footerText || "Bereitgestellt von BookMe"}</span><span>Sichere Buchungsverwaltung</span></footer>; }
