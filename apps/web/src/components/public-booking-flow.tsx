...
export function PublicBookingFlow({ slug }: { slug: string }) {
  ...
  return <div className="public-page">
    <header className="public-header"><div className="public-workspace-brand"><PublicBrandLogo branding={branding} /><strong>{branding?.workspaceName || "BookMe"}</strong></div><div><Icon name="globe" /><select value={timezone} onChange={(item) => { setLoadingSlots(true); resetScheduleProgress(); setTimezone(item.target.value); }} aria-label="Zeitzone für die Buchung">{timeZones.map((zone) => <option value={zone} key={zone}>{timeZoneLabel(zone)}</option>)}</select></div></header>
    ...
    <footer className="public-footer"><span>{branding?.footerText || "Bereitgestellt von BookMe"}</span><span>Sichere Terminplanung</span></footer>
  </div>;
}
