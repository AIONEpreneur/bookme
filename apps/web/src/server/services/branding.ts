...
export async function setBranding(workspaceId: string, userId: string, input: WorkspaceBranding) {
  ...
  return db.$transaction(async (tx) => {
    ...
    if (canonicalLogo && isRemoteImageUrl(canonicalLogo) && canonicalLogo !== existing?.logoUrl) throw new AppError("INVALID_IMAGE", "Externe Bild-URLs können nicht gespeichert werden. Lade stattdessen die Bilddatei hoch.", 422, { logoUrl: ["Externe Bild-URLs können nicht gespeichert werden. Lade stattdessen die Bilddatei hoch."] });
